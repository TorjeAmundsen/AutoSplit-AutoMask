...
public partial class MainWindow : Window
{
    ...
    // Markers AutoMask writes into generated filenames: _(threshold), _[pause], _#delay#, _{d}, _{b}.
    // A base image whose name contains one of these was almost certainly generated by AutoMask.
    private static readonly Regex GeneratedFilenameMarkers = new(@"_\(\d+(\.\d+)?\)|_\[\d+(\.\d+)?\]|_#\d+#|_\{[db]\}", RegexOptions.Compiled);
    ...
    private async void BtnLoadInputImages_Click(object sender, RoutedEventArgs e)
    {
        ...
        // The file picker returns files in selection order; split pairing relies on
        // alphabetical order (AutoSplit saves screenshots as 000_..., 001_..., etc.).
        var inputPaths = files.Select(f => f.Path.LocalPath)
            .OrderBy(Path.GetFileName, StringComparer.OrdinalIgnoreCase)
            .ToList();

        InputLoadingOverlay.IsVisible = true;
        var (previewCache, thumbCache, transparentPaths) = await Task.Run(() =>
        {
            var previews = new ConcurrentDictionary<string, Bitmap>();
            var thumbs = new ConcurrentDictionary<string, Bitmap>();
            var transparent = new ConcurrentBag<string>();
            Parallel.ForEach(inputPaths, path =>
            {
                using var sk = SKBitmap.Decode(path);
                if (HasNonOpaquePixel(sk))
                {
            ...
            return (new Dictionary<string, Bitmap>(previews), new Dictionary<string, Bitmap>(thumbs), transparent.ToHashSet());
        });
        InputLoadingOverlay.IsVisible = false;

        // Generated split images loaded as base images would be overwritten and moved
        // to /base by Save all. AutoSplit screenshots are fully opaque and carry no
        // AutoMask filename markers, so either signal means the user picked wrong files.
        var suspects = inputPaths
            .Where(p => transparentPaths.Contains(p) || GeneratedFilenameMarkers.IsMatch(Path.GetFileName(p)))
            .ToList();

        if (suspects.Count > 0)
        {
            var result = await MessageBox.Show(this, "Warning",
                "These images look like AutoMask output images, not base screenshots (they contain transparency or AutoMask filename markers):\n\n"
            ...
            {
                foreach (var b in previewCache.Values)
                {
                    b.Dispose();
                }

                foreach (var b in thumbCache.Values)
                {
                    b.Dispose();
                }
    ...
    private static bool HasNonOpaquePixel(SKBitmap bitmap)
    {
    ...
    private async void BtnSaveAllSplits_Click(object sender, RoutedEventArgs e)
    {
        ...
        var movedCount = await MoveBaseImagesToSubfolder(inputPaths, outDir);

        ShowStatus(movedCount > 0
            ? $"Saved {inputPaths.Count} images, moved base images to /base"
            : $"Saved {inputPaths.Count} images");
    }

    // AutoSplit reads every image in the folder it's pointed at, so base images
    // can't stay in the output folder next to the generated split images.
    private async Task<int> MoveBaseImagesToSubfolder(List<string> inputPaths, string outDir)
    {
        var normalizedOutDir = Path.GetFullPath(outDir)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        var baseDir = Path.Combine(normalizedOutDir, "base");
        var selectedIndex = ComboBoxSelectInputImage.SelectedIndex;
        var movedCount = 0;
        var errors = new List<string>();

        for (var i = 0; i < inputPaths.Count; i++)
        {
            var oldPath = inputPaths[i];
            var parentDir = Path.GetDirectoryName(Path.GetFullPath(oldPath));

            if (!string.Equals(parentDir, normalizedOutDir, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }

            var newPath = Path.Combine(baseDir, Path.GetFileName(oldPath));
            ...
            if (_inputPreviewCache.Remove(oldPath, out var preview))
            {
                _inputPreviewCache[newPath] = preview;
            }

            if (_inputThumbnailCache.Remove(oldPath, out var thumbnail))
            {
                _inputThumbnailCache[newPath] = thumbnail;
            }

            if (string.Equals(_selectedInputImagePath, oldPath, StringComparison.OrdinalIgnoreCase))
            {
                _selectedInputImagePath = newPath;
            }

            if (i < inputImagesComboBoxItems.Count)
            {
                inputImagesComboBoxItems[i] = inputImagesComboBoxItems[i] with { FullPath = newPath };
            }
        }

        if (movedCount > 0)
        {
            if (ComboBoxSelectInputImage.SelectedIndex != selectedIndex)
            {
                ComboBoxSelectInputImage.SelectedIndex = selectedIndex;
            }
        ...
        if (errors.Count > 0)
        {
            await ShowMessage("Error", "Failed to move base image(s) to /base.", detail: string.Join(Environment.NewLine, errors));
        }
